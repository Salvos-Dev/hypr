...
if [[ ! -z $CURR_SSID ]]; then
	HIGHLINE=$(echo "$(echo "$IW_NETWORKS" | awk -F "[  ]{2,}" '{print $1}' | grep -Fxn -m 1 "$CURR_SSID" | awk -F ":" '{print $1}') + 1")
fi
...
CHENTRY=$(echo -e "$MENU" | uniq -u | rofi -i -dmenu -p "󰤨 " -lines "$LINE_COUNT" -a "$HIGHLINE" -location "$POSITION" -yoffset "$Y_OFF" -xoffset "$X_OFF" -font "$FONT" -width -"$R_WIDTH")
